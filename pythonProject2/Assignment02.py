...
# analyze_boolean_table. As your starting point, use the enhanced version from class on September 6.
# Omit the part about tt_type (tautology, contingency, contradiction). You may want to reverse the binary
# string (using binary[::-1]) before converting it to a decimal number, as that may better align the result with the
# function number.
...
# [4] Rewrite functions f0 thru f15 from Assignment 1 as 0/1 boolean functions.
# It is not necessary to include the other functions from that assignment (De Morgan's Laws, Modus Ponens,
# Modus Tollens, and Hypothetical Syllogism).
...
# Use a dictionary to keep track of which functions resulted in which binary number. Verify that all 16 numbers were
# each produced once and only once.


def analyze_functions():
    functions = [f0_false, f1_nor, f2_converse_nonimpl, f3_neg_p, f4_nimply, f5_neg_q, f6_xor,
                 f7_nand, f8_and, f9_xnor, f10_q, f11_imply, f12_p, f13_converse, f14_or, f15_True]
    data = []
...
def half_adder():
 ...
 #   c = b_and(x,y)
    data = [[x, y, b_and(b_or(x, y), b_not(b_and(x, y))), b_and(x, y)] for x in [0, 1] for y in [0, 1]]
    headers = ["x", "y", "s", "c"]
    title = "Half Adder"
    alignments = ["r", "r", "r", "r"]
...
def reverse_engineer(f):
    ...
    function = " + ".join(["".join([tt_heads[j] + ("'" if tt.iloc[i][j] == 0 else "") for j in range(tt_vars)])
                           for i in range(tt_rows) if tt_out[i] == 1])
    return function
