...
def analyze_truth_table(f, verbose=False):
    ...
    binary = "".join(["1" if value else "0" for value in last_col])[::-1]
    decimal = int(binary, 2)
    if verbose:
        print("Name:", f.__name__, func_body(f))
        print(tt)
        print("Rows:", tt_rows, "Cols:", tt_cols, "Vars:", tt_vars, "Type:", tt_type)
    return tt_rows, tt_cols, tt_vars, tt_type, last_col, binary, decimal

#[7] Apply the analyze function to the sixteen possible functions listed at https://en.wikipedia.org/wiki/Truth_table#Truth_table_for_all_binary_logical_operators. Label them f0 thru f15, to match the order in Wikipedia. All sixteen should have two propositional variables p and q, even if the function uses only one or the other. Verify that the output matches what is expected.
...
def f9_xor(p,q):
    return (p and q) and (not p and not q)
...
def main():
    ...
    print(func_body(dm1l), func_body(dm2l), is_equivalent(dm1l, dm1l))  # True
    print(func_body(dm2l), func_body(dm2r), is_equivalent(dm2l, dm2r))  # False

    functions = [f0_false, f1_nor, f2_converse_nonimpl, f3_neg_p, f4_nimply, f5_neg_q,
                 f6_xor, f7_nand, f8_and, f9_xor, f10_q, f11_imply, f12_p, f13_conv, f14_or, f15_True]
