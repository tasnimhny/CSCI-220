...
# function "func" and two sets -
# domain and co_domain and determines whether the function passed in is "partial",
# "bijectve", "injective", or "surjective". First,
# compute func_range by evaluating the function for each value of the domain.
...
# The first five functions are binary and should accept two
# arguments each, while the last function is unary and should accept only one argument.
# You may use built-in functions to your language or
...
# [c] Compare the results (since there may be roundoff error, do not
# look for equality but rather a difference less than 1)
def q3():
    n = 10
    a = 5
    r = 6
...
# [5] Define a function generate_binary_number(binary_numbers) that takes a list of the string representation of
# some fractional binary numbers in the range [0,1) and uses Cantor's diagonalization argument to find a number not
# already in that list. Be mindful of the case where there are more elements in the list than digits in each number,
